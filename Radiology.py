...
if oncall_template:
    ...
    with st.expander("⚙️ Scheduling Preferences (Optional)", expanded=False):
        ...
        st.markdown("#### 4. Additional Preference Requests")
        st.caption("Add custom requests for specific rads and days")
        ...
        try:
            with st.spinner("Generating optimized schedule..."):
                ...
                with st.expander("📊 View Details"):
                    ...
                    st.write(f"**GEN assignments:** {gen_count}")
                    st.write(f"**IRA assignments:** {ira_count}")
                    st.write(f"**MRI assignments:** {mri_count}")
