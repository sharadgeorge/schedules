    ...
    if st.button("🔄 Convert to Import Format", type="primary"):
        try:
            with st.spinner("Processing Cardiology schedules..."):
                ...
                # Debug: Check month parameter
                st.info(f"🔍 Processing {calendar.month_name[selected_month]} {selected_year} (month={selected_month})")
                ...
                cardio_data = cardio_converter.read_cardiovascular_data(
                    wb_cardio1,      # Pass workbook, not worksheet!
                    selected_year,
