...
if cardio_file1 and cardio_file2:
    ...
    st.info(f"📅 Selected values - Month: {selected_month} (type: {type(selected_month).__name__}), Year: {selected_year}")
    ...
    if st.button("🔄 Convert to Import Format", type="primary"):
        try:
            with st.spinner("Processing Cardiology schedules..."):
                ...
                # Load workbooks (not data_only so we can modify B4)
                ...
                # CRITICAL FIX: The converter reads the month from cell B4 in the worksheets
                # We need to ensure B4 has the correct month name for both files
                
                month_name = calendar.month_name[int(selected_month)]
                st.info(f"🔧 Setting cell B4 to: {month_name}")
                
                # Find and set B4 in sheets that contain the month name
                for sheet_name in wb_cardio1.sheetnames:
                    if month_name.lower() in sheet_name.lower() or 'nov' in sheet_name.lower():
                        ...
                        ws['B4'] = month_name
                        st.success(f"✅ Set B4 in '{sheet_name}' (Cardiovascular)")
                        break
                ...
                for sheet_name in wb_cardio2.sheetnames:
                    if month_name.lower() in sheet_name.lower() or 'nov' in sheet_name.lower():
                        ...
                        ws['B4'] = month_name  
                        st.success(f"✅ Set B4 in '{sheet_name}' (Interventional)")
                        break
                
                # Debug: Check month parameter before calling converter
                st.info(f"🔍 About to call converter with:")
                st.info(f"  - Year: {selected_year} (type: {type(selected_year).__name__})")
                st.info(f"  - Month: {selected_month} (type: {type(selected_month).__name__})")
                st.info(f"  - Month name should be: {calendar.month_name[selected_month]}")
                
                # Call converter functions - IMPORTANT: Pass workbook (wb), not worksheet (ws)
                # The converter will find the correct sheet based on month
                
                # Ensure month and year are proper integers
                month_int = int(selected_month)
                year_int = int(selected_year)
                
                st.info(f"🔍 Calling converter with: year={year_int}, month={month_int}")
                ...
                try:
                    ...
                    st.error(f"❌ IndexError in converter: {str(e)}")
                    st.error(f"This usually means the month value is out of range (should be 1-12)")
                    st.error(f"We passed: month={month_int}, year={year_int}")
                    st.error("The converter might be reading the month from cell B4 in the Excel file instead of using our parameter.")
                    st.error("Please check cell B4 in your Excel file - it should contain the month number (1-12).")
                    raise
                except Exception as e:
                    st.error(f"❌ Error in read_cardiovascular_data: {str(e)}")
