...
class OnCallScheduler:
    def __init__(self, excel_path):
        ...
        self.MRI_ROWS = MRI_ROWS  # Make accessible as instance variable
        self.MRI_RADS = MRI_RADS  # Make accessible as instance variable
        self.GEN_ROWS = GEN_ROWS  # Make accessible as instance variable
        self.IRA_ROWS = IRA_ROWS  # Make accessible as instance variable
    ...
    def assign_gen_thursday_saturday(self):
        ...
        for day in range(1, self.days_in_month + 1):
            ...
            for rad in available_rads:
                ...
                base_score = thu_score + sat_score
                
                # WEEKEND OPTIMIZATION: Bonus for MRI-capable rads on weekends
                # Lower score = better, so subtract bonus to prefer MRI-capable rads
                if rad in self.MRI_RADS:
                    mri_bonus = 150
                    scores[rad] = base_score - mri_bonus
                    print(f"    Weekend MRI bonus for {rad}: -{mri_bonus} (score: {base_score:.1f} -> {scores[rad]:.1f})")
                else:
                    scores[rad] = base_score
    ...
    def assign_ira_triplets(self):
        ...
        for day in range(1, self.days_in_month + 1):
            ...
            for rad in available_rads:
                ...
                # WEEKEND OPTIMIZATION: Bonus for MRI-capable rads on weekend triplets
                # Lower score = better, so subtract bonus to prefer MRI-capable rads
                mri_weekend_bonus = 0
                if rad in self.MRI_RADS:
                    mri_weekend_bonus = 200
                    print(f"    Weekend triplet MRI bonus for {rad}: -{mri_weekend_bonus}")
                
                scores[rad] = base_score + consecutive_penalty + weekend_penalty - mri_weekend_bonus
    ...
    def generate_schedule(self):
        ...
        # CAPTURE MRI QUALITY METRICS
        print("\n" + "="*80)
        print("MRI ASSIGNMENT WITH QUALITY ASSESSMENT")
        print("="*80)
        self.mri_quality_metrics = self.assign_mri_3rad_days_only()
    ...
    def assign_mri_3rad_days_only(self):
        ...
        # Step 2.5: WEEKEND TRIPLET CONSOLIDATION (NEW OPTIMIZATION)
        # If an MRI-capable rad is on GEN for 2 of 3 days in a weekend triplet,
        # assign that rad to cover MRI for the remaining day as well
        # This converts partial 3-rad weekends to 2-rad weekends
        print("\n=== Weekend Triplet Consolidation (New Optimization) ===")
        ...
        for day in range(1, self.days_in_month + 1):
            if self.get_day_type(day) == 'thu':
                ...
                sat_day = day + 2
                if sat_day <= self.days_in_month:
                    # Check if this weekend has any 3-rad days
                    days_in_triplet = [day, fri_day, sat_day]
                    triplet_3rad_days = [d for d in days_in_triplet if d in three_rad_days]
                    
                    if len(triplet_3rad_days) > 0:  # Has at least one 3-rad day
                        # Check if GEN has an MRI-capable rad on 2+ days
                        gen_assignments = {
                            day: self.assignments['GEN'].get(day),
                            fri_day: self.assignments['GEN'].get(fri_day),
                            sat_day: self.assignments['GEN'].get(sat_day)
                        }
                        
                        # Count which MRI-capable rads appear on GEN and how many days
                        mri_capable_counts = {}
                        for d, rad in gen_assignments.items():
                            if rad and rad in self.MRI_RADS:
                                mri_capable_counts[rad] = mri_capable_counts.get(rad, 0) + 1
                        
                        # Find if any rad appears 2+ times (covering 2 of 3 days)
                        consolidation_rad = None
                        for rad, count in mri_capable_counts.items():
                            if count >= 2:
                                consolidation_rad = rad
                                break
                        
                        if consolidation_rad:
                            # Assign this rad to cover MRI for ALL 3 days of weekend
                            print(f"  Weekend {day}-{sat_day}: {consolidation_rad} on GEN {count} days")
                            print(f"    → Consolidating: {consolidation_rad} covers MRI for entire weekend")
                            
                            for d in days_in_triplet:
                                if d in triplet_3rad_days:  # Only assign MRI for 3-rad days
                                    self.assignments['MRI'][d] = consolidation_rad
                                    self.monthly_counts[consolidation_rad][self.get_day_type(d)] += 1
                                    self.mri_monthly_total[consolidation_rad] += 1
                                    consolidated_days.append(d)
                                    # Remove from 3-rad days list since we've handled it
                                    three_rad_days.remove(d)
                                    if d in three_rad_weekdays:
                                        three_rad_weekdays.remove(d)
                            
                            # Update quality metrics
                            print(f"    → Result: 2-rad weekend (efficient!) ✅")
        
        # Recount 3-rad weekends after consolidation
        three_rad_weekends_after = []
        for day in range(1, self.days_in_month + 1):
            if self.get_day_type(day) == 'thu':
                fri_day = day + 1
                sat_day = day + 2
                if sat_day <= self.days_in_month:
                    if day in three_rad_days and fri_day in three_rad_days and sat_day in three_rad_days:
                        three_rad_weekends_after.append((day, fri_day, sat_day))
        
        three_rad_weekends = three_rad_weekends_after
        quality_metrics['three_rad_weekends'] = len(three_rad_weekends)
        quality_metrics['three_rad_weekdays'] = len(three_rad_weekdays)
        quality_metrics['three_rad_days'] = len(three_rad_days)
        
        if len(consolidated_days) > 0:
            print(f"\n✅ Consolidated {len(consolidated_days)} days via weekend triplet optimization")
            print(f"   New totals: {len(three_rad_days)} 3-rad days, {len(three_rad_weekends)} 3-rad weekends")
        
        if len(three_rad_days) == 0:
            print("✓ No remaining 3-rad days! Perfect optimization!")
            quality_metrics['optimization_level'] = 'PERFECTLY OPTIMIZED ⭐⭐⭐'
            return quality_metrics
        ...
        # Determine optimization level (prioritize zero 3-rad weekends!)
        weekend_count = len(three_rad_weekends)
        weekday_count = len(three_rad_weekdays)
        
        if weekend_count == 0 and weekday_count == 0:
            quality_metrics['optimization_level'] = 'PERFECTLY OPTIMIZED ⭐⭐⭐'
        elif weekend_count == 0 and weekday_count <= 3:
            quality_metrics['optimization_level'] = 'WELL OPTIMIZED ⭐⭐'
        elif weekend_count == 0 and weekday_count <= 6:
            quality_metrics['optimization_level'] = 'GOOD OPTIMIZATION ⭐'
        elif weekend_count == 1:
            quality_metrics['optimization_level'] = 'MODERATELY OPTIMIZED ⚠️'
        else:
            quality_metrics['optimization_level'] = 'POORLY OPTIMIZED ⚠️⚠️'
    ...
    def calculate_ytd_variance(self, ws_read=None):
        ...
        # GEN section - use actual GEN_ROWS mapping
        print("\n--- GEN Section Variance ---")
        for rad in GEN_RADS:
            if rad == 'TA':  # Skip TA (non-participating)
                continue
            if rad not in self.GEN_ROWS:
                continue
        ...
        # IRA section - use actual IRA_ROWS mapping
        print("\n--- IRA Section Variance ---")
        for rad in IRA_RADS:
            if rad not in self.IRA_ROWS:
                continue
        ...
        # MRI section - Skip variance calculation (no MRI targets per user requirement)
        # User note: "we do not need to calculate variances in MRI because there are no YTD targets for MRI"
        print("\n--- MRI Section: Skipped (no YTD targets for MRI) ---")
        
        # Calculate aggregate variance metrics (GEN + IRA only)
        aggregate_variance = {
